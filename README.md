...
- 🎯 **视觉指向** — 结合 VLM（视觉语言模型）截屏分析，三角形光标会飞向 AI 识别到的目标位置
- 🗣️ **语音唤醒** — 说出唤醒词（默认"贾维斯"）即可开始对话，无需动手
- 🎤 **语音识别 (STT)** — 基于 SenseVoice 离线语音转文字，隐私安全
- 🔊 **语音合成 (TTS)** — 对接本地 TTS 服务，AI 回复自动朗读
- 🌀 **状态可视化** — 聆听时绿色呼吸圆球，思考时紫色呼吸圆球，待机时静默三角形
- ⚙️ **Win11 风格设置面板** — Fluent Design 三栏标签页，支持实时光标坐标显示与定点测试
...
```
语音唤醒 ──→ STT识别 ──→ 截屏+构建Prompt ──→ VLM推理 ──→ 解析回复
                                                            │
                              ┌────────────────────────────┤
                              ↓              ↓              ↓
                         气泡显示       光标飞行指向      TTS朗读
...
> **注意**：首次运行会自动创建 `mocli.db` 配置数据库。语音模型目录不存在时，程序会优雅退化为纯文本模式。
